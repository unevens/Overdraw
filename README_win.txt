...
INSTALL — STEP 1: UNBLOCK THE ZIP (only if downloaded from the web)
  ...
  Web" flag that can cause your DAW to refuse to load Overdraw or
...
INSTALL — STEP 2: COPY THE PLUG-IN
  ...
  Copy the "Overdraw 1.2.vst3" folder (the whole folder, not just
  the file inside it) to ONE of the locations below. Use the
  per-user path if you want to install for just your account, or
  the system path if you want every user on this PC to see Overdraw.

  Per-user (no admin needed):
    %LOCALAPPDATA%\Programs\Common\VST3\Overdraw 1.2.vst3

  System-wide (admin required):
    C:\Program Files\Common Files\VST3\Overdraw 1.2.vst3
...
INSTALL — STEP 3: RESCAN IN YOUR DAW
  ...
  before Overdraw shows up in the plug-in menu.


WHY NO CODE SIGNATURE?

  Overdraw is free, open-source software. Distributing it with a
