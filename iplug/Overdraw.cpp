...
// Shared IVStyle for every Overdraw panel control — knobs, switches,
// tabs, meters. Dark base, cyan accents, slim 1px frames.
static const IVStyle kOverdrawStyle = DEFAULT_STYLE
...
// =============================================================================
// OverdrawMeterControl — IVMeterControl with light-coloured dB markers.
// Upstream's DrawMarkers hardcodes DEFAULT_TEXT (black) which is unreadable
// on Overdraw's dark panel. We reimplement Draw() to use a light text style
// for the "-24 dB" / "0 dB" / etc. tick labels alongside each meter.
// =============================================================================
template<int MAXNC>
class OverdrawMeterControl : public IVMeterControl<MAXNC>
{
    ...
    auto attachLabeledKnob =
      [&](const IRECT& cell, int paramIdx, const char* name) -> IControl* {
        pGraphics->AttachControl(new ITextControl(
          knobLabelRectIn(cell), name, kOverdrawStyle.labelText));
