...
VST binaries are available at https://www.unevens.net/overdraw.html.


You can also build Overdraw as a VST3 plugin, but there it has an issue with Ableton Live when reporting the latency during the plugin initialization, which can lead the a crash. The branch `vst3-latency-workaround` offers a work in progress fix for that, but for now I suggest to use the VST version.
